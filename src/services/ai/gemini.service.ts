...
async function retryWithBackoff<T>(
  ...
  try {
    ...
    if (isAuthError(error)) {
      throw new Error(
        `AI authentication failed (check GOOGLE_API_KEY). Original error: ${getErrorMessage(error)}`
      );
    }

    if (isQuotaExceeded(error)) {
      throw new Error(
        `AI quota exceeded for the current plan/model. Upgrade billing or wait for quota reset. Original error: ${getErrorMessage(error)}`
      );
    }
