...
export async function vectorSearch(embedding, workspaceId, limit = 20, filters) {
    try {
      ...
      LIMIT $2
    `;
        const results = await prisma.$queryRawUnsafe(query, `[${embedding.join(',')}]`, limit);
...
export async function hybridSearch(embedding, keywords, workspaceId, limit = 20) {
    try {
        const query = `
      SELECT 
        ki.id,
      ...
      LIMIT $4
    `;
        const results = await prisma.$queryRawUnsafe(query, `[${embedding.join(',')}]`, keywords, workspaceId, limit);
